...
import com.qiqi.jiaoyou.mapper.BankcardMapper;
...
public class BankcardServiceImpl extends ServiceImpl<BankcardMapper, Bankcard> implements IBankcardService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<Bankcard> wherePage = new Page<>(page, limit);
        QueryWrapper<Bankcard> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<Bankcard> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(Bankcard bankcard){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(bankcard);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(Bankcard bankcard){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(bankcard);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        Bankcard obj = baseMapper.selectById(id);
