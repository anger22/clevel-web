...
import com.qiqi.jiaoyou.mapper.NewFriendMapper;
...
public class NewFriendServiceImpl extends ServiceImpl<NewFriendMapper, NewFriend> implements INewFriendService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<NewFriend> wherePage = new Page<>(page, limit);
        QueryWrapper<NewFriend> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<NewFriend> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(NewFriend newFriend){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(newFriend);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(NewFriend newFriend){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(newFriend);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        NewFriend obj = baseMapper.selectById(id);
