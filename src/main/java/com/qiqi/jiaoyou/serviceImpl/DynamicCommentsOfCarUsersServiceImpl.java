...
import com.qiqi.jiaoyou.mapper.DynamicCommentsOfCarUsersMapper;
...
public class DynamicCommentsOfCarUsersServiceImpl extends ServiceImpl<DynamicCommentsOfCarUsersMapper, DynamicCommentsOfCarUsers> implements IDynamicCommentsOfCarUsersService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<DynamicCommentsOfCarUsers> wherePage = new Page<>(page, limit);
        QueryWrapper<DynamicCommentsOfCarUsers> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<DynamicCommentsOfCarUsers> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(DynamicCommentsOfCarUsers dynamicCommentsOfCarUsers){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(dynamicCommentsOfCarUsers);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(DynamicCommentsOfCarUsers dynamicCommentsOfCarUsers){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(dynamicCommentsOfCarUsers);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        DynamicCommentsOfCarUsers obj = baseMapper.selectById(id);
