...
import com.qiqi.jiaoyou.mapper.ProblemMapper;
...
public class ProblemServiceImpl extends ServiceImpl<ProblemMapper, Problem> implements IProblemService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<Problem> wherePage = new Page<>(page, limit);
        ...
        IPage<Problem> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(Problem problem){
        ResultUtils resultUtils = new ResultUtils();
        problem.setAddTime(new Timestamp(System.currentTimeMillis()));
        problem.setEnableState(1);
        problem.setDeleteState(2);
        Integer i =  baseMapper.insert(problem);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(Problem problem){
        ResultUtils resultUtils = new ResultUtils();
        problem.setEditTime(new Timestamp(System.currentTimeMillis()));
        Integer i =  baseMapper.updateById(problem);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        Problem obj = baseMapper.selectById(id);
    ...
    public ResultUtils updateEnable(Integer id, Integer enableState) {
        ...
        problem.setId(id);
        problem.setEnableState(enableState);
        int i = baseMapper.updateById(problem);
        if (i > 0){
