...
public class MemberAssetsServiceImpl extends ServiceImpl<MemberAssetsMapper, MemberAssets> implements IMemberAssetsService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<MemberAssets> wherePage = new Page<>(page, limit);
        QueryWrapper<MemberAssets> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<MemberAssets> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(MemberAssets memberAssets){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(memberAssets);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(MemberAssets memberAssets){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(memberAssets);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        MemberAssets obj = baseMapper.selectById(id);
    ...
    @Override
    public MemberAssets findByMemberId(Long id) {
       QueryWrapper<MemberAssets> wrapper = new QueryWrapper<>();
