...
import com.qiqi.jiaoyou.mapper.RiderDynamicsMapper;
...
public class RiderDynamicsServiceImpl extends ServiceImpl<RiderDynamicsMapper, RiderDynamics> implements IRiderDynamicsService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<RiderDynamics> wherePage = new Page<>(page, limit);
        QueryWrapper<RiderDynamics> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<RiderDynamics> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(RiderDynamics riderDynamics){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(riderDynamics);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(RiderDynamics riderDynamics){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(riderDynamics);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        RiderDynamics obj = baseMapper.selectById(id);
