...
import com.qiqi.jiaoyou.mapper.RechargeRecordMapper;
...
public class RechargeRecordServiceImpl extends ServiceImpl<RechargeRecordMapper, RechargeRecord> implements IRechargeRecordService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<RechargeRecord> wherePage = new Page<>(page, limit);
        QueryWrapper<RechargeRecord> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<RechargeRecord> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(RechargeRecord rechargeRecord){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(rechargeRecord);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(RechargeRecord rechargeRecord){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(rechargeRecord);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        RechargeRecord obj = baseMapper.selectById(id);
