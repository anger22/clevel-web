...
import com.qiqi.jiaoyou.mapper.LevelMapper;
...
public class LevelServiceImpl extends ServiceImpl<LevelMapper, Level> implements ILevelService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<Level> wherePage = new Page<>(page, limit);
        QueryWrapper<Level> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<Level> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(Level level){
        ResultUtils resultUtils = new ResultUtils();
        level.setAddTime(new Timestamp(System.currentTimeMillis()));
        Integer i =  baseMapper.insert(level);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(Level level){
        ResultUtils resultUtils = new ResultUtils();
        level.setEditTime(new Timestamp(System.currentTimeMillis()));
        Integer i =  baseMapper.updateById(level);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        Level obj = baseMapper.selectById(id);
