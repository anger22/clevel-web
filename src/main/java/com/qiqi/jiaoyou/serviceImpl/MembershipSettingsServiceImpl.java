...
import com.qiqi.jiaoyou.mapper.MembershipSettingsMapper;
...
public class MembershipSettingsServiceImpl extends ServiceImpl<MembershipSettingsMapper, MembershipSettings> implements IMembershipSettingsService {
    ...
    public ResultUtils findListByPage(Integer page, Integer limit,Map<String,String>map){
        ...
        IPage<MembershipSettings> wherePage = new Page<>(page, limit);
        QueryWrapper<MembershipSettings> wrapper = Wrappers.query();
        for (String key : map.keySet()) {
            wrapper.like(key,map.get(key));
        }
            wrapper.orderByDesc("id");

        IPage<MembershipSettings> list =   baseMapper.selectPage(wherePage, wrapper);
    ...
    @Override
    public ResultUtils add(MembershipSettings membershipSettings){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.insert(membershipSettings);
        if(i > 0){
            resultUtils.setStatus(200);
    ...
    @Override
    public ResultUtils updateData(MembershipSettings membershipSettings){
        ResultUtils resultUtils = new ResultUtils();
        Integer i =  baseMapper.updateById(membershipSettings);
        if(i > 0){
            resultUtils.setStatus(200);
            resultUtils.setMessage("编辑成功");
        }else{
    ...
    public ResultUtils findById(Long id){
        ResultUtils resultUtils = new ResultUtils();
        MembershipSettings obj = baseMapper.selectById(id);
